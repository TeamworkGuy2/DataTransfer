...
public interface ProtocolInput extends Closeable {
	 ...
	 * @throws IOException if the next stream element's id does not match the id specified or
	...
	/** Parse a short value from the next element.
	 * @param id the id of the element to read
	...
	/** Read an opening block tag from this input stream.
	 ...
	 * @throws IOException if the opening block id read does not match the id specified or
	 * if there is an IO error while reading from the input stream
	 */
	 ...
	 * The closing block tag read is compared to the last opening block tag
	 * read by this stream and if they do not match, an exception is thrown.
	 * @throws IOException if the closing block id read does not match the last opening block id specified or
	 * if there is an IO error reading from the input stream
	 */
	public void readClosingBlock() throws IOException;


	/** Get the current header block tag
	...
	/** Get the number of blocks currently read by this reader.
	 * @return the number of blocks {@code (opening + closing tags)/2} read by this reader
	 */
