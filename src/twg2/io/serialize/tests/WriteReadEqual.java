package twg2.io.serialize.tests;
...
public class WriteReadEqual {
	...
	public void checkWriteRead() throws IOException {
		...
		checkWriteReadOfFormat(DataTransferFormat.XML, charset);
		checkWriteReadOfFormat(DataTransferFormat.JSON, charset);
	}


	public void checkWriteReadOfFormat(DataTransferFormat format, Charset charset) throws IOException {
		String formatName = format.name().toLowerCase();
		String fileName = formatName + "_test." + formatName;
		File file = new File(fileName);

		//JsonTest twg2.io.serialize.tests = new JsonTest(file, charset);
