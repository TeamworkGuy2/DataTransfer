...
public interface XmlInput extends Closeable {
	...
	/** Read an array of bytes from the next XML element of this input stream.
	 * @param name the name of the XML element to read
	 * @param b the byte array to store the bytes read from this XML stream in.
	 * The length of the byte array determines how many bytes are read.
	 * Bytes are added to the array starting at index 0.
	 * @throws IOException if the specified name does not match the name of the element read or
	 * if there is an IO or XML error reading from the input stream
	 */
	public void read(String name, byte[] b) throws IOException;


	/** Read the specified number of bytes from the next XML element of this input stream.
	 * @param name the name of the XML element to read
	 * @param b the byte array to store the bytes read from this XML stream in
	 * @param off the offset into the byte array to store the bytes read from this XML stream
	 * @param len the number of bytes to read from this XML stream and write into the byte array
	 * @throws IOException if the specified name does not match the name of the element read or
	 * if there is an IO or XML error reading from the input stream
	 */
	...
	/** Read the next opening or closing XML block tag.
	 * @throws IOException if there is an IO or XML error reading from the input stream
	 */
	...
	/** Read a closing XML block tag.
	 * @throws IOException if the next read closing tag does not match the last read opening tag or
	 * if there is an IO or XML error reading from the input stream
	 */
